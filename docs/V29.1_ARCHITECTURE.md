...
### 7.1 Flight Deck & Cockpit (`:8787`)

| Path | Responsibility |
|------|----------------|
| `src/cockpit/web_server.py` | FastAPI static + WebSocket hub @ 2.5 Hz |
| `src/cockpit/telemetry_bridge.py` | Snapshot collector, co-pilot vitals, OBI, shadow MTM |
| `src/cockpit/telemetry_schema.py` | `IgPositionTelemetry`, `OrderBookDepthPayload` |
| `src/cockpit/position_drift.py` | Broker vs local cache drift |
| `cockpit-web/` | Flight Deck UI (index.html, app.js, styles.css) |

### 7.2 Intelligence Plane (v29.1 Phase 2)

| Path | Responsibility |
|------|----------------|
| `src/intelligence/microstructure.py` | Multi-TF classifier, velocity RSI override, warmup blend |
| `src/intelligence/alpha_trail.py` | ATR trail + **Capital Harvesting contract** |
| `src/trading/alpha_trail.py` | Trading-plane facade for harvest contract |
| `src/intelligence/target_engine.py` | £1k daily target, parabolic milestone snap |
| `src/intelligence/macro_radar.py` | DXY / 10Y proxy correlation weights |
| `src/intelligence/velocity_filter.py` | 200ms tick burst / 90% override |
| `src/intelligence/intelligence_worker.py` | Non-blocking numpy compute thread |
| `src/trading/shadow_executor.py` | `IG_AGENT_MODE=SHADOW` ledger |
| `src/system/thread_affinity.py` | M-series P-core / QoS pinning |
| `src/system/self_healing_supervisor.py` | Patch-branch gate + hot reload |
...
| Layer | Location |
|-------|----------|
| Unit / integration | `tests/` (~860+ tests) |
| Stress gates S1–S3 | `tests/stress/` |
| Cockpit avionics | `tests/test_cockpit_avionics.py` |
| Infinite edge / harvest | `tests/test_infinite_edge_overhaul.py`, `tests/test_intelligence_alpha_trail.py` |
| Deploy regression | `tests/test_deployed_fixes.py` (67) |
...
|---------|-------------------|
| v29.1 Flight Deck sync (Jun 2026) | Per-asset avionics markets (`cockpit/avionics_markets.py`), broker closed-trade hydration, Card B isolated gauges + BLOCKER/AI WEIGHT, Decimal-safe WS sanitize |
| v29.1 Phase 2 (Jun 2026) | Flight Deck co-pilot, intelligence plane, capital harvesting, shadow executor, stress S1–S3, thread affinity |
