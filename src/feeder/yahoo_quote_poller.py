...
from data.ohlc_yahoo_seeder import (
    EPIC_YAHOO_MAP,
    default_spread_for_yahoo_symbol,
    yahoo_symbols_for_epic,
)
from system.engine_log import log_engine
from system.market_data_hub import QuoteSnapshot, get_market_data_hub

_YAHOO_CHART_TEMPLATE = "https://{host}/v8/finance/chart/{symbol}"
...
def fetch_yahoo_mid(symbol: str, *, timeout_sec: float = _DEFAULT_TIMEOUT_SEC) -> float | None:
    ...
    for host in _YAHOO_CHART_HOSTS:
        url = _YAHOO_CHART_TEMPLATE.format(
            host=host,
            symbol=url_quote(symbol, safe=""),
        )
        try:
            ...
            last_exc = exc
            err = str(exc).lower()
            if host != _YAHOO_CHART_HOSTS[-1] and (
...
def fetch_yahoo_quote(epic: str, *, timeout_sec: float = _DEFAULT_TIMEOUT_SEC) -> YahooQuoteSample | None:
    for symbol in yahoo_symbols_for_epic(epic):
