...
def _seed_db(path: Path) -> None:
    ...
    rows = [
        ("2026-06-10 08:00:00", "2026-06-10 09:00:00", "Gold", "CS.D.CFPGOLD.CFP.IP", "BUY", 100, 110, 1, 95, 115, 10, "WIN", 0, 50.0, "agent|buy"),
        ("2026-06-10 10:00:00", "2026-06-10 11:00:00", "Gold", "CS.D.CFPGOLD.CFP.IP", "BUY", 100, 95, 1, 95, 115, -5, "LOSS", 0, -25.0, "agent|buy"),
        ("2026-06-10 12:00:00", "2026-06-10 13:00:00", "Gold", "CS.D.CFPGOLD.CFP.IP", "SELL", 100, 90, 1, 105, 85, 10, "WIN", 0, 40.0, "agent|sell"),
        ("2026-06-10 14:00:00", "2026-06-10 15:00:00", "SIM", "CS.D.CFPGOLD.CFP.IP", "BUY", 100, 120, 1, 95, 115, 20, "WIN", 1, 999.0, "test_setup"),
        ("2026-06-10 08:30:00", "2026-06-10 09:30:00", "Dow", "IX.D.DOW.IFM.IP", "BUY", 100, 90, 1, 95, 115, -10, "LOSS", 0, -30.0, "agent|buy"),
    ]
    conn.executemany(
        """
        INSERT INTO trades (
            opened_at, closed_at, market, epic, side, entry, exit, size, stop, target,
            pnl_points, result, dry_run, ig_pnl_currency, setup_key
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        rows,
    )
...
class EdgeAnalysisTests(unittest.TestCase):
    ...
    def test_edge_analysis_excludes_sim_trades(self) -> None:
        payload = compute_edge_analysis(self.tmp.name)
